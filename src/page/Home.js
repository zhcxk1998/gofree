...
class Home extends Component {
  ...
  handleFormSubmit = () => {
    const {
      isLogin, formType, formStartDay, formEndDay, formDes,
    } = this.state;
    if (!isLogin) {
      ...
      const path = {
        pathname: '/lines',
        data: {
          type: formType, start: formStartDay, end: formEndDay, des: formDes,
        },
      };
  ...
  handleLoginOut = () => {
    const $this = this;
    axios.get('/account/logout/api').then((res) => {
...
开始定制
        ...
        {/** 视频模态框* */}
        {videoModalOpen
          ? <VideoModal modalOpen videoUrl={videoUrl} closeVideoModal={this.closeVideoModal} /> : null}
        {/** 登录模态框* */}
        {loginModalOpen
          ? (
            <LoginModal
        ...
        {preferencesModalOpen
          ? <PreferencesModal modalOpen finalRegisterHandle={this.finalRegisterHandle} /> : null}
        <Footer />
      </div>
    );
  }
