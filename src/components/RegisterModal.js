...
export default class RegisterModal extends Component {
  ...
  clearWarning = () => {
    const {
      password, confirmPassword, phone, verifyCode,
    } = this.state;
    this.setState({
  ...
  handleVerifyBtn =() => {
    ...
    this.countDown(60);
    axios.get('/account/veri_sms/api', { params: { phone } })
      .then((res) => {
        console.log(res);
      });
  }
  ...
  _registerHandle = () => {
    const {
      username, password, confirmPassword, phone, verifyCode,
    } = this.state;
  ...
  render() {
    const {
      phoneWarning, verifyNote, verifyBtn, modalOpen, username, password, confirmPassword, phone, verifyCode, usernameValid, passwordValid, passwordConfirmValid, phoneValid, verifyCodeValid, passwordWarning,
    } = this.state;
    return (
      <div>
        <Modal
        ...
        >
          ...
          <Modal.Content>
            <Form>
              ...
              />
                {this.state.phoneWarning ? <p style={{ color: 'red', fontSize: '8px' }}>请输入有效手机号</p> : <p />}
              </Form.Field>
              <Form.Field required>
                <label>验证码</label>
              ...
              <Form.Field required>
                ...
                error={passwordValid}
                onBlur={() => this.setState({ passwordLenWarning: password.length < 4 })}
              />
                {this.state.passwordLenWarning ? <p style={{ color: 'red', fontSize: '8px' }}>密码最短长度为4位</p> : <p />}
              </Form.Field>
              <Form.Field required>
                <label>确认密码</label>
                <Form.Input
                ...
                onBlur={() => this.setState({ passwordWarning: password !== confirmPassword })}
                onChange={(e) => {
              ...
              />
                {this.state.passwordWarning ? <p style={{ color: 'red', fontSize: '8px' }}>请保持输入的密码一致</p> : <p />}
              </Form.Field>
            </Form>
          </Modal.Content>
          <Modal.Actions>
