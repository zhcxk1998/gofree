...
class PreferencesModal extends Component {
  ...
  componentDidMount() {
    const $this = this;
    axios.get('/persona/getCandidatePlaces/3301/').then((res) => {
      const _rec = res.result;
  ...
  handleSelect = (idx) => {
    const { items } = this.state;
    const temp = items[idx].isSelected;
...
做一个直接的外貌党，哪些图片是你喜欢的风格,
            <span style={{ color: 'green' }}>不求你专一,可以选多个哦</span>
          </p>
          <Grid>
            <Grid.Row columns={3}>
              {items.map((item, idx) => (
                  ...
                  <Card>
                    ...
                    <Card.Content>
                      <Card.Header>
                        {item.name}
                        {' '}
                        {item.isSelected ? <Icon name="check circle outline" style={{ fontSize: '1.1em' }} color="green" /> : null}
                      </Card.Header>
                      <Card.Description>
                      </Card.Description>
                    </Card.Content>
